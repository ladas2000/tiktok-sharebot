## THIS TOOL WAS MADE BY NIGHTFALL
## EDITED BY WOOX | ALL CREDITS GO TO NIGHTFALL
    ...
    try:
        ...
        try:
            if (req.json()["status_code"] == 0):
                ...
                TotalSendedShare += 1
                if DebugMode == True:
                    print(Colorate.Horizontal(Colors.green_to_white, f"[{TotalSendedShare}] Share Sent!"))
                else:
                    print(Colorate.Horizontal(Colors.green_to_white, f"[{TotalSendedShare}] Share Sent!"))
...
fucked up the ui my bad lol -- wx made this and hz skidded it lol

"""
    print(Colorate.Horizontal(Colors.red_to_white, banner))
    print("\n \n")
    itemID       = Write.Input("Video Link (vm.tiktok.com and tiktok.com/video/blahblah) works > ", Colors.red_to_white, interval=0.0001)
    amount       = Write.Input("Amount of shares(0=inf) > ", Colors.red_to_white, interval=0.0001)
    NThread      = Write.Input("How many threads do you want (100 is usually good) > ", Colors.red_to_white, interval=0.0001)
    
    if Title("Proy Scrapper X-Proxy by NightFallGT (skidded by hz lol xd uwu owo") == True:
